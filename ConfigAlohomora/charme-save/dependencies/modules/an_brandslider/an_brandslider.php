...
// @codingStandardsIgnoreEnd
{
    ...
    protected $params = array(
        ...
        'imageType' => array(
                ...
                'type'=> 'imageType',
                'hint' => '',
                'desc' => "If you want to use an ImageType that has been created recently and hasn't been used yet, please run its regeneration. Otherwise, some images won't be displayed correctly.",
            ),
        'margin' => array(
                'label'=>'margin',
                'value' => 0,
    ...
    public function __construct()
    {
		...
		$this->addons_product_id = '25991';
		
		
        Module::__construct();

        $this->displayName = $this->l('Brand Slider: partners, manufacturers logo carousel');
    ...
    public function displayContent($params = null)
    {
        ...
        if (isset($img_format)) {
			...
			foreach ($items as $item){
				foreach ($_manufacturers as $manufacture){
					if ($item == $manufacture['id_manufacturer']){
						$manufacturers[$manufacture['id_manufacturer']] = $manufacture;
					}
				}
			}
    ...
    public function getContent()
    {
        ...
        return $this->topNav() . $this->displayForm();
    }
    ...
    {
        $hookName = lcfirst(str_replace('hook', '', $function));
	...
	public function topNav(){
		...
		$this->context->smarty->assign(array(
			'settingUrl' => $this->context->link->getAdminLink('AdminModules').'&configure=an_shippingmatrix',
			'costsUrl' => $this->context->link->getAdminLink('AdminAnShippingMatrix'),
			'zonesUrl' => $this->context->link->getAdminLink('AdminAnshippingmatrixZones'),
			'AddCarrierUrl' => $this->context->link->getAdminLink('AdminAnshippingmatrixCarrier'),
