...
*}
    ...
    <div id="mobile-amegamenu">
      ...
      <li class="amenu-item mm-{$menu.id_anmenu} {if $menu.dropdowns}plex{/if}">
        ...
        {if $menu.link}<a href="{$menu.link}" class="amenu-link {if Module::isEnabled('an_theme') and Module::getInstanceByName('an_theme')->getParam('topmenu_mobileLinks') == 0 }amenu-nolink{/if}">{else}<span class="amenu-link">{/if}
          <span>{$menu.name}</span>
            ...
            {elseif $dropdown.content_type == 'product'}
              ...
              {foreach from=$dropdown.products item=product name=products}
                <div class="product-item">
                  <p class="product-thumbnail"><a href="{$product.url}" title="{$product.name}"><img class="img-fluid" src="{$product.cover.bySize.menu_default.url}" width="auto" height="auto" alt="{$product.cover.legend}" /></a></p>
                  <div class="product-information-dropdown">
                    <p class="h5 product-name"><a href="{$product.url}" title="{$product.name}">{$product.name|truncate:25:'...'}</a></p>
