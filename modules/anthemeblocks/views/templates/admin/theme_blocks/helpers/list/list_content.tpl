        ...
        {foreach $fields_display AS $key => $params}
            {block name="open_td"}
                <td
					...
					{*   {if !in_array(0, $tr.disabled_actions)}  if no children  {/if}
            ...
            {block name="td_content"}
                ...
                {if isset($tr.$key)}
                    ...
                    {elseif isset($params.activeVisu)}
                        {if $tr.$key}
                            <i class="icon-check-ok"></i> {l s='Enabled' mod='anthemeblocks'}
                        {else}
                            <i class="icon-remove"></i> {l s='Disabled' mod='anthemeblocks'}
                        {/if}
                    ...
                    {elseif isset($params.type) && $params.type == 'percent'}
                        {$tr.$key} {l s='%' mod='anthemeblocks'}
                    {elseif isset($params.type) && $params.type == 'bool'}
            {if $tr.$key == 1}
              {l s='Yes' mod='anthemeblocks'}
            {elseif $tr.$key == 0 && $tr.$key != ''}
              {l s='No' mod='anthemeblocks'}
            {/if}
    ...
    {if $has_actions}
        <td class="text-right">
            ...
            {foreach $actions AS $key => $action}
                ...
                {/if}
            {/foreach}
            {if $compiled_actions|count > 0 and $compiled_actions|count > $tr.disabled_actions|count}
                {if $compiled_actions|count > 1}<div class="btn-group-action">{/if}
                <div class="btn-group pull-right">
                    {if in_array(0, $tr.disabled_actions)}
                        {assign var=tempo value=array_shift($compiled_actions)}
                        {array_shift($compiled_actions)|replace:'class="edit':'class="btn btn-default edit'}
    ...
    <tr>
        <td class="list-empty" colspan="{count($fields_display)+1}">
            <div class="list-empty-msg">
                ...
                {l s='No records found' mod='anthemeblocks'}
            </div>
        </td>
    </tr>
{/if}
