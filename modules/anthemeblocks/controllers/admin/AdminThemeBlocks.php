...
class AdminThemeBlocksController extends ModuleAdminController
{
    ...
    public function renderList() {
		...
		$this->context->smarty->assign('theme', $this->module->getThemeInfo());
		
        $_content = $this->context->smarty->createTemplate(
            _PS_MODULE_DIR_.'anthemeblocks/views/templates/admin/promo.tpl', null, null, $this->context->smarty
        )->fetch();
        return $_content.parent::renderList();
    }
...
/*         $image = $obj->getImage();
		...
		foreach ($languages as $lang) {
			$this->fields_value['image'][$lang['id_lang']] = $this->object->image[$lang['id_lang']];
		}
		//////////////////////////////////////////////
    ...
    public function getList($id_lang, $order_by = null, $order_way = null, $start = 0, $limit = null, $id_lang_shop = false)
    {
        ...
        foreach ($this->_list as &$list) {
            if ($list['image'] !='' && Tools::file_exists_no_cache(_PS_MODULE_DIR_.'anthemeblocks/images/'.$list['image'])) {
                $list['image'] = '<img style="max-width: 75px; max-height: 75px" src="../modules/anthemeblocks/images/'.$list['image'].'?rand='.rand(1, 1000).'" alt="" class="imgm img-thumbnail" />';
            } else {
    ...
    public function processSave()
    {
		...
		if (isset($object->id) && $object->id) {
			...
			$uniqName = uniqid();
			$ext = 'jpg';
			$path = AnThemeBlock::getImagesPath();
			
			foreach ($languages as $lang) {
				
                if (isset($_FILES['image_'.$lang['id_lang']])
                    && isset($_FILES['image_'.$lang['id_lang']]['tmp_name'])
                    ...
                    //    $ext = substr($_FILES['image_'.$lang['id_lang']]['name'], strrpos($_FILES['image_'.$lang['id_lang']]['name'], '.') + 1);
                        $file_name = $uniqName . '_' . $lang['id_lang'] . '.'.$ext;

                        if (!move_uploaded_file($_FILES['image_'.$lang['id_lang']]['tmp_name'], $path .$file_name)) {
							...
							@unlink($path . $object->image[$lang['id_lang']]);
							$object->image[$lang['id_lang']] = $file_name;
						}
                    }
